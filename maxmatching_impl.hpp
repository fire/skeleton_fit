...
getMaxMatching(vector<int> labels1, vector<int> labels2){
	...
	//add constraints
	Variable* varnos = new Variable[varMap.size()];
	Variable* varweights = new Variable[varMap.size()];		
	for (int32_t i = 0; i < varMap.size(); i++) {
		solver.addEditVariable(varweights[i], strength::strong);
		solver.addEditVariable(varnos[i], strength::strong);
	}
	...
	for (set<int>::iterator it1 = l1set.begin(); it1 != l1set.end(); it1++){
		...
		for (set<int>::iterator it2 = l2set.begin(); it2 != l2set.end(); it2++){
			...
			solver.addConstraint(Constraint{ varweights[j] <= 1 });
			solver.suggestValue(varnos[j], varMap[pair<int,int>(*it1, *it2)]);
			solver.addConstraint(Constraint{ varnos[j] <= 1 });
			j++;
		}
	}
	...
	for (map<pair<int, int>, int>::iterator it = varMap.begin(); it != varMap.end(); it++){
		solver.suggestValue(varnos[j], it->second);
		solver.suggestValue(varweights[j], weightMap[it->second]);
		j++;
	}
	...
	for (int i = 0; i < invvarMap.size(); i++){
		if (fabs(varweights[i].value() - 1.0) < 1e-6){
